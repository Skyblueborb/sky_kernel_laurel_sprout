...
int max98937_reg_channel_map[][7][2] = {
	...
	},
	{
		...
		{ MAX98937_PCM_to_speaker_monomix_A,
		  0x80 }, //TODO: check Quin's
		{ MAX98937_PCM_to_speaker_monomix_B,
		  0x00 }, //TODO: check Quin's
	},
	{
		// right channel
...
#define DSM_SET_STEREO_PARAM(cmdId)                                            \
	((cmdId & 0x00FFFFFF) | DSM_API_STEREO_SPKER)
...
{
	if (!max98927 || max98927->rcv_mix_mode || max98927->factory_test ||
	    !max98927->dsm_enable) {
...
static ssize_t maxdsm_read(struct file *filep, char __user *buf, size_t count,
			   loff_t *ppos)
{
	int rc;
...
static struct miscdevice dsm_ctrl_miscdev = { .minor = MISC_DYNAMIC_MINOR,
					      .name = "dsm_ctrl_dev",
					      .fops = &dsm_ctrl_fops };
#endif
					     ...
					     size_t count, loff_t *ppos)
{
	...
	if (ret == 0) {
		...
		if (!rdc_check_valid(impedance_l, MAX98927L)) {
			impedance_l =
				SPK_MUTE_VALUE; //calibration failed specail code
...
static ssize_t max989xx_dbgfs_f0_read(struct file *file, char __user *user_buf,
				      size_t count, loff_t *ppos)
{
	struct i2c_client *i2c = file->private_data;
	struct max989xx_priv *max98927 = i2c_get_clientdata(i2c);
					  ...
					  loff_t *ppos)
{
	...
	*ppos = pos;
	...
	n += scnprintf(kbuf + n, sizeof(kbuf) - n, "%d",
		       !!max98927_get_calib_status(MAX98927R));
...
static void max989xx_debug_init(struct max989xx_priv *max98927,
				struct i2c_client *i2c)
{
	...
	debugfs_create_file("status", S_IRUGO, max98927->dbg_dir, i2c,
			    &max989xx_dbgfs_status_fops);
	//bug 487235 zhanghao1 20190917 Add Use different boardID loading different audio parameters begin
	debugfs_create_file("boardid", S_IRUGO, max98927->dbg_dir, i2c,
			    &max989xx_dbgfs_boardid_fops);
	//bug 487235 zhanghao1 20190917 Add Use different boardID loading different audio parameters end
}
...
void max98927_wrapper_write(struct max989xx_priv *max98927, unsigned int reg,
			    unsigned int val)
{
	int i;
...
void max98927_wrap_update_bits(struct max989xx_priv *max98927, unsigned int reg,
	...
	for (i = 0; i < MAX_CHANNEL_NUM; i++) {
		if (i2c_states & (1 << i)) {
			...
			case SND_SOC_DAIFMT_CBS_CFS:
				regmap_update_bits(
					max98927->regmap[i], pcm_master_mode,
			...
			case SND_SOC_DAIFMT_NB_NF:
				regmap_update_bits(
					max98927->regmap[i], pcm_mode_config,
			...
			case SND_SOC_DAIFMT_LEFT_J:
				...
				break;
			default:
				pr_info("DAI interface unsupported %x, %x",
					SND_SOC_DAIFMT_I2S,
...
static const int rate_table[] = {
	5644800,  6000000,  6144000,  6500000,	9600000,
	11289600, 12000000, 12288000, 13000000, 19200000,
};
	...
	for (j = 0; j < MAX_CHANNEL_NUM; j++) {
		if (i2c_states & (1 << j)) {
			...
			if (max98927->master) {
				...
				regmap_update_bits(
					max98927->regmap[j], pcm_master_mode,
			...
			pr_info("%s: BLCK fix to %d\n", __func__,
				blr_clk_ratio);
			regmap_update_bits(
				max98927->regmap[j], pcm_clock_setup,
				  ...
				  struct snd_soc_dai *dai)
{
	...
	for (i = 0; i < MAX_CHANNEL_NUM; i++) {
		if (i2c_states & (1 << i)) {
			if (max98927->bIsMax98937[i]) {
				pcm_mode_config = MAX98937_PCM_Mode_Config;
				pcm_sample_rate_setup_1 =
					MAX98937_PCM_Sample_rate_setup_1;
				pcm_sample_rate_setup_2 =
					MAX98937_PCM_Sample_rate_setup_2;
				pr_info("max98937 %s: format supported",
					__func__);
			} else {
				pr_info("max98927 %s: format supported",
					__func__);
			}
			...
			case 32:
				...
				break;
			default:
				pr_err("%s: format unsupported %d", __func__,
			...
			pr_info("%s: %s stream, format: %d, ch: %d, sampling_rate: %d\n",
				...
				max98927->ch_size, params_channels(params),
				params_rate(params));

			/* set DAI_SR to correct LRCLK frequency */
			regmap_update_bits(
				max98927->regmap[i], pcm_sample_rate_setup_1,
				MAX98927_PCM_Sample_rate_setup_1_DIG_IF_SR_Mask,
				sampling_rate);
			regmap_update_bits(
				max98927->regmap[i], pcm_sample_rate_setup_2,
				MAX98927_PCM_Sample_rate_setup_2_SPK_SR_Mask,
				sampling_rate << 4);
...
static int max98927_dai_set_sysclk(struct snd_soc_dai *dai, int clk_id,
...
static int max98927_stream_mute(struct snd_soc_dai *codec_dai, int mute,
				int stream)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
		...
		if (mute) {
			...
			for (i = 0; i < MAX_CHANNEL_NUM; i++) {
				if (i2c_states & (0x1 << i)) {
					if (max98927->bIsMax98937[i]) {
						global_enable =
							MAX98937_Global_Enable;
						amp_enable =
							MAX98937_AMP_enables;
					}
					regmap_update_bits(max98927->regmap[i],
							   global_enable, 1, 0);
					regmap_update_bits(max98927->regmap[i],
					...
					if (max98927->safe_gain) {
						if (!max98927->dsm_enable) { // feedback is disable so dsm algorithm is incompleted , so reduce gain to safe value to protect speaker by power ouput control
							regmap_update_bits(
								...
								MAX98927_AMP_volume_control_AMP_VOL_Mask,
								max98927->digital_safe_gain);
							regmap_update_bits(
								max98927->regmap
									[i],
								spk_gain,
								MAX98927_Speaker_Gain_SPK_PCM_GAIN_Mask,
								max98927->spk_safe_gain);
						}
					}
					regmap_update_bits(max98927->regmap[i],
							   amp_enable, 1, 1);
		...
		for (i = 0; i < MAX_CHANNEL_NUM; i++) {
			if (i2c_states & (1 << i)) {
				if (max98927->bIsMax98937[i])
					regmap_update_bits(
						max98927->regmap[i],
						MAX98937_Speaker_Gain,
						MAX98937_Speaker_Gain_SPK_PCM_GAIN_Mask,
						sel);
				else
					regmap_update_bits(
						max98927->regmap[i],
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (sel <= ((1 << MAX98927_AMP_VOL_WIDTH) - 1)) {
		max98927_wrap_update_bits(
			max98927, MAX98927_AMP_volume_control,
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!max98927->bIsMax98937[MAX98927L])
		...
		return max98927_reg_get(kcontrol, ucontrol,
					MAX98937_Boost_Control_0,
					MAX98937_Boost_Control_0_BST_VOUT_Mask,
					0);
}
					  ...
					  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!max98927->bIsMax98937[MAX98927L])
		return max98927_reg_get(kcontrol, ucontrol,
					MAX98927_Boost_Control_1,
					MAX98927_Boost_Control_1_BST_ILIM_Mask,
					MAX98927_BST_ILIM_SHIFT);
	else
		return max98927_reg_get(kcontrol, ucontrol,
					MAX98937_Boost_Control_1,
					MAX98937_Boost_Control_1_BST_ILIM_Mask,
					MAX98927_BST_ILIM_SHIFT);
}
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
	if (!max98927->bIsMax98937[MAX98927L])
		return max98927_reg_get(
			kcontrol, ucontrol, MAX98927_Speaker_source_select,
			MAX98927_Speaker_source_select_SPK_SOURCE_Mask, 0);
	else
		return max98927_reg_get(
			kcontrol, ucontrol, MAX98937_Speaker_source_select,
	...
	for (i = 0; i < MAX_CHANNEL_NUM; i++) {
		if (i2c_states & (1 << i)) {
			if (max98927->bIsMax98937[i])
				regmap_update_bits(
					max98927->regmap[i],
					MAX98937_Speaker_source_select,
					MAX98937_Speaker_source_select_SPK_SOURCE_Mask,
					sel);
			else
				regmap_update_bits(
					max98927->regmap[i],
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!max98927->bIsMax98937[MAX98927L])
		return max98927_reg_get(
			kcontrol, ucontrol, MAX98927_PCM_to_speaker_monomix_A,
			MAX98927_PCM_to_speaker_monomix_A_DMONOMIX_CH0_SOURCE_Mask,
			0);
	else
		return max98927_reg_get(
			kcontrol, ucontrol, MAX98937_PCM_to_speaker_monomix_A,
			MAX98937_PCM_to_speaker_monomix_A_DMONOMIX_CH0_SOURCE_Mask,
			0);
}
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (i2c_states & MAX98927_CH0) {
		if (!max98927->bIsMax98937[MAX98927L]) {
			...
			regmap_read(max98927->regmap[MAX98927L],
				    MAX98937_PCM_to_speaker_monomix_A, &data);
			ucontrol->value.integer.value[0] =
				(data &
				 MAX98937_PCM_to_speaker_monomix_A_DMONOMIX_CH0_SOURCE_Mask);
		}
	...
	if (i2c_states & MAX98927_CH0) {
		if (!max98927->bIsMax98937[MAX98927L]) {
			regmap_update_bits(
				...
				MAX98927_PCM_to_speaker_monomix_A_DMONOMIX_CH0_SOURCE_Mask,
				sel);
			regmap_update_bits(max98927->regmap[MAX98927L],
					   MAX98927_PCM_Rx_Enables_A, 0xf,
					   sel + 1);
		} else {
			regmap_update_bits(
				max98927->regmap[MAX98927L],
				...
				sel);
			regmap_update_bits(max98927->regmap[MAX98927L],
					   MAX98937_PCM_Rx_Enables_A, 0xf,
					   sel + 1);
		}
		pr_info("%s: register 0x%02X, value 0x%02X\n", __func__,
			MAX98927_PCM_to_speaker_monomix_A, sel);
	}
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (i2c_states & MAX98927_CH1) {
		if (!max98927->bIsMax98937[MAX98927R]) {
			regmap_read(max98927->regmap[MAX98927R],
				    MAX98927_PCM_to_speaker_monomix_A, &data);
			ucontrol->value.integer.value[0] =
				(data &
				 MAX98927_PCM_to_speaker_monomix_A_DMONOMIX_CH0_SOURCE_Mask);
	...
	if (i2c_states & MAX98927_CH1) {
		if (!max98927->bIsMax98937[MAX98927R]) {
			...
			regmap_update_bits(max98927->regmap[MAX98927R],
					   ...
					   sel + 1);
			pr_info("%s: register 0x%02X, value 0x%02X\n", __func__,
				MAX98937_PCM_to_speaker_monomix_A, sel);
		}
	} else {
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (i2c_states & MAX98927_CH0) {
		if (!max98927->bIsMax98937[MAX98927L]) {
			regmap_read(max98927->regmap[MAX98927L],
				    MAX98927_Measurement_enables, &data);
		} else {
			regmap_read(max98927->regmap[MAX98927L],
				    MAX98937_Measurement_enables, &data);
		}
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (i2c_states & MAX98927_CH0) {
		if (!max98927->bIsMax98937[MAX98927L]) {
			regmap_write(max98927->regmap[MAX98927L],
				     MAX98927_Measurement_enables, sel);
			pr_info("%s: register 0x%02X, value 0x%02X\n", __func__,
				MAX98927_Measurement_enables, sel);
		} else {
			regmap_write(max98927->regmap[MAX98927L],
				     MAX98937_Measurement_enables, sel);
			pr_info("%s: register 0x%02X, value 0x%02X\n", __func__,
				MAX98937_Measurement_enables, sel);
		}
	}
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (i2c_states & MAX98927_CH1) {
		if (!max98927->bIsMax98937[MAX98927R]) {
			regmap_write(max98927->regmap[MAX98927R],
				     MAX98927_Measurement_enables, sel);
			pr_info("%s: register 0x%02X, value 0x%02X\n", __func__,
				MAX98927_Measurement_enables, sel);
		} else {
					    ...
					    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (i2c_states & MAX98927_CH0) {
		if (!max98927->bIsMax98937[MAX98927L]) {
			regmap_read(max98927->regmap[MAX98927L],
				    MAX98927_Global_Enable, &data_global);
			regmap_read(max98927->regmap[MAX98927L],
				    MAX98927_AMP_enables, &data_amp);
			ucontrol->value.integer.value[0] =
				(data_global & MAX98927_Global_Enable_EN) &
				(data_amp & MAX98927_AMP_enables_SPK_EN);
		} else {
			regmap_read(max98927->regmap[MAX98927L],
				    MAX98937_Global_Enable, &data_global);
			regmap_read(max98927->regmap[MAX98927L],
				    MAX98937_AMP_enables, &data_amp);
			ucontrol->value.integer.value[0] =
				(data_global & MAX98937_Global_Enable_EN) &
				(data_amp & MAX98937_AMP_enables_SPK_EN);
		}
	}

	pr_info("%s: value:%d", __func__,
		(int)ucontrol->value.integer.value[0]);
	return 0;
}
					    ...
					    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (i2c_states & MAX98927_CH0) {
		if (!max98927->bIsMax98937[MAX98927L]) {
			pr_info("%s: register 0x%02X, value 0x%02X\n", __func__,
				MAX98927_Global_Enable, sel);
			regmap_update_bits(max98927->regmap[MAX98927L],
					   MAX98927_AMP_enables, 1, sel);
			regmap_update_bits(max98927->regmap[MAX98927L],
					   MAX98927_Global_Enable, 1, sel);

		} else {
			pr_info("%s: register 0x%02X, value 0x%02X\n", __func__,
				MAX98937_Global_Enable, sel);
			regmap_update_bits(max98927->regmap[MAX98927L],
					   MAX98937_AMP_enables, 1, sel);
			regmap_update_bits(max98927->regmap[MAX98927L],
					   MAX98937_Global_Enable, 1, sel);
		}
	}
	return 0;
}
...
static int
max98927_right_channel_enable_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
...
static int
max98927_right_channel_enable_set(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
	...
	if (i2c_states & MAX98927_CH1) {
		if (!max98927->bIsMax98937[MAX98927R]) {
			...
			pr_info("%s: Force %s speaker, register 0x%02X, value 0x%02X\n",
				...
				MAX98927_Global_Enable, disable);
		} else {
			if (disable) {
				regmap_update_bits(max98927->regmap[MAX98927R],
						   MAX98937_Global_Enable, 1,
						   1);
			} else {
				regmap_update_bits(max98927->regmap[MAX98927R],
						   MAX98937_Global_Enable, 1,
			...
			pr_info("%s: Force %s speaker, register 0x%02X, value 0x%02X\n",
				__func__, disable ? "disable " : "enable",
				MAX98937_Global_Enable, disable);
		}
	}
					  ...
					  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!max98927->bIsMax98937[MAX98927L]) {
		if (mix) {
			...
			for (i = 0; i < ARRAY_SIZE(max98927_reg_channel_map[0]);
			     i++)
				regmap_write(max98927->regmap[MAX98927L],
					     max98927_reg_channel_map[0][i][0],
			...
			for (i = 0;
			     i <
			     ARRAY_SIZE(
				     max98927_reg_channel_map[MAX98927L + 1]);
			     i++)
				regmap_write(
					max98927->regmap[MAX98927L],
					max98927_reg_channel_map[MAX98927L + 1]
		...
		if (mix) {
			pr_info("%s: Enter Mix Mode\n", __func__);
			for (i = 0; i < ARRAY_SIZE(max98937_reg_channel_map[0]);
			     i++)
				regmap_write(max98927->regmap[MAX98927L],
					     max98937_reg_channel_map[0][i][0],
			...
			for (i = 0;
			     i <
			     ARRAY_SIZE(
				     max98937_reg_channel_map[MAX98927L + 1]);
			     i++)
				regmap_write(
					max98927->regmap[MAX98927L],
					max98937_reg_channel_map[MAX98927L + 1]
...
static const char *const max98927_boost_current_limit_text[] = {
	"1.0A", "1.1A", "1.2A", "1.3A", "1.4A", "1.5A", "1.6A", "1.7A",
	"1.8A", "1.9A", "2.0A", "2.1A", "2.2A", "2.3A", "2.4A", "2.5A",
	"2.6A", "2.7A", "2.8A", "2.9A", "3.0A", "3.1A", "3.2A", "3.3A",
	"3.4A", "3.5A", "3.6A", "3.7A", "3.8A", "3.9A", "4.0A", "4.1A"
};

static const char *const max98927_speaker_source_text[] = { "i2s", "reserved",
							    "tone", "pdm" };

static const char *const max98927_monomix_output_text[] = { "ch_0", "ch_1",
							    "ch_1_2_div" };

static const char *const max98927_feedback_switch_text[] = { "OFF", "V_EN",
							     "I_EN", "ON" };

static const char *const max98927_receiver_mix_text[] = { "OFF", "ON" };
...
static const struct soc_enum max98927_enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(max98927_feedback_switch_text),
			    max98927_feedback_switch_text),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(max98927_boost_current_limit_text),
			    max98927_boost_current_limit_text),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(max98927_receiver_mix_text),
			    max98927_receiver_mix_text),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(max98927_adsp_working_mode_text),
			    max98927_adsp_working_mode_text),
};

static const struct snd_kcontrol_new max98927_snd_controls[] = {
	SOC_SINGLE_EXT_TLV("Speaker Volume", MAX98927_Speaker_Gain, 0,
			   (1 << MAX98927_Speaker_Gain_Width) - 1, 0,
			   max98927_spk_gain_get, max98927_spk_gain_put,
			   max98927_spk_tlv),
	SOC_SINGLE_EXT_TLV("Left Speaker Volume", MAX98927_Speaker_Gain, 0,
			   (1 << MAX98927_Speaker_Gain_Width) - 1, 0,
			   max98927_spk_gain_get_l, max98927_spk_gain_put_l,
			   max98927_spk_tlv),
	SOC_SINGLE_EXT_TLV("Right Speaker Volume", MAX98927_Speaker_Gain, 0,
			   (1 << MAX98927_Speaker_Gain_Width) - 1, 0,
			   max98927_spk_gain_get_r, max98927_spk_gain_put_r,
			   max98927_spk_tlv),
	//000:mute	001:+3db  010:+6db	011:+9db  100:+12db  101:+15db	110:+18db  111:reserved
	SOC_SINGLE_EXT_TLV("Digital Gain", MAX98927_AMP_volume_control, 0,
			   (1 << MAX98927_AMP_VOL_WIDTH) - 1, 0,
			   max98927_digital_gain_get, max98927_digital_gain_put,
			   max98927_digital_tlv),
	SOC_SINGLE_EXT_TLV("Left Digital Gain", MAX98927_AMP_volume_control, 0,
			   (1 << MAX98927_AMP_VOL_WIDTH) - 1, 0,
			   max98927_digital_gain_get_l,
			   max98927_digital_gain_put_l, max98927_digital_tlv),
	SOC_SINGLE_EXT_TLV("Right Digital Gain", MAX98927_AMP_volume_control, 0,
			   (1 << MAX98927_AMP_VOL_WIDTH) - 1, 0,
			   max98927_digital_gain_get_r,
			   max98927_digital_gain_put_r, max98927_digital_tlv),
	//0x00~0x7f:-16db ~ 15.75db
	SOC_SINGLE_EXT("BDE Enable", MAX98927_Brownout_enables, 0, 1, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	//0:disable 1:enable
	SOC_SINGLE_EXT("Amp DSP Enable", MAX98927_Brownout_enables,
		       MAX98927_BDE_DSP_SHIFT, 1, 0, max98927_reg_get_w,
		       max98927_reg_put_w),
	//0:disable 1:enable
	SOC_SINGLE_EXT("BDE AMP Enable", MAX98927_Brownout_enables, 1, 1, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	//0:disable 1:enable
	SOC_SINGLE_EXT("Ramp Switch", MAX98927_AMP_DSP_Config,
		       MAX98927_SPK_RMP_EN_SHIFT, 1, 1, max98927_reg_get_w,
		       max98927_reg_put_w),
	//Control for Volume Ramp during Startup and Shutdown 0:on 1:bypass
	SOC_SINGLE_EXT("DRE EN", MAX98927_DRE_Control, 0, 1, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	...
	SOC_SINGLE_EXT("Level4 Threshold", MAX98927_Brownout__level_4_threshold,
		       0, 255, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 255
	SOC_SINGLE_EXT("Level1 Current Limit",
		       MAX98927_Brownout__level_1_current_limit, 0, 63, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 63
	SOC_SINGLE_EXT("Level2 Current Limit",
		       MAX98927_Brownout__level_2_current_limit, 0, 63, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 63
	SOC_SINGLE_EXT("Level3 Current Limit",
		       MAX98927_Brownout__level_3_current_limit, 0, 63, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 63
	SOC_SINGLE_EXT("Level4 Current Limit",
		       MAX98927_Brownout__level_4_current_limit, 0, 63, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 63
	...
	SOC_ENUM_EXT("Boost Current Limit", max98927_enum[4],
		     max98927_boost_input_limit_get,
		     max98927_boost_input_limit_put),
	//booset current limit
	SOC_ENUM_EXT("Speaker Source", max98927_enum[1], max98927_spk_src_get,
		     max98927_spk_src_put),
	//speaker source
	SOC_ENUM_EXT("Monomix Output", max98927_enum[0], max98927_mono_out_get,
	...
	//channel select
	//should divide left and right channel?
	SOC_SINGLE_EXT("Left Channel Enable", MAX98927_Global_Enable, 0, 1, 0,
		       max98927_left_channel_enable_get,
		       max98927_left_channel_enable_set),
	//0:disable 1:enable
	SOC_SINGLE_EXT("Right Channel Enable", MAX98927_Global_Enable, 0, 1, 0,
		       max98927_right_channel_enable_get,
		       max98927_right_channel_enable_set),
	SOC_SINGLE_EXT("Speaker Force Close", MAX98927_Global_Enable, 0, 1, 0,
		       max98927_speaker_force_close_get,
		       max98927_speaker_force_close_set),
	//Set receiver into mix mode or not
	SOC_ENUM_EXT("Receiver Mix Mode", max98927_enum[5],
		     max98927_receiver_mix_mode_get,
		     max98927_receiver_mix_mode_put),
...
static int max98927_parse_dt(struct max989xx_priv *max98927, struct device *dev,
			     int id)
{
	...
	if (!of_property_read_u32(dNode, "mono_stereo_mode", &value)) {
		...
		max98927->mono_stereo =
			value; // 0: mono 1: left only 2: right only 3: stereo
	}
	...
	if (IS_ERR(max98927->max989xx_vdd)) {
		...
		if (regulator_count_voltages(max98927->max989xx_vdd) > 0) {
			ret = regulator_set_voltage(max98927->max989xx_vdd,
						    1800000, 1800000);
			if (ret) {
			...
			ret = regulator_set_load(max98927->max989xx_vdd,
						 200000);
			if (ret) {
	...
	if (id == MAX98927L) {
		max98927->reset_gpio_l =
			of_get_named_gpio(dNode, "maxim,98927-reset-gpio", 0);
			      ...
			      const struct i2c_device_id *id)
{
	...
	if (!max98927) {
		max98927 =
			devm_kzalloc(&i2c->dev, sizeof(*max98927), GFP_KERNEL);
		if (!max98927) {
	...
	if (IS_ERR(max98927->regmap[id->driver_data])) {
		...
		switch (check_max98927_presence(
			max98927->regmap[id->driver_data])) {
		case MAX98927_ID:
			regmap_reinit_cache(max98927->regmap[id->driver_data],
					    &max98927_regmap);
			max98927->bIsMax98937[id->driver_data] = MAX98927_ID;

			for (i = 0;
			     i < ARRAY_SIZE(max98927_reg_channel_map[idx]); i++)
				regmap_write(
					max98927->regmap[id->driver_data],
					max98927_reg_channel_map[idx][i][0],
					max98927_reg_channel_map[idx][i][1]);

			for (i = 0; i < ARRAY_SIZE(max98927_reg_common_map);
			     i++)
				regmap_write(max98927->regmap[id->driver_data],
					     max98927_reg_common_map[i][0],
					     max98927_reg_common_map[i][1]);
			break;
		case MAX98937_ID:
			regmap_reinit_cache(max98927->regmap[id->driver_data],
					    &max98937_regmap);
			...
			for (i = 0;
			     i < ARRAY_SIZE(max98937_reg_channel_map[idx]); i++)
				regmap_write(
					max98927->regmap[id->driver_data],
					max98937_reg_channel_map[idx][i][0],
					max98937_reg_channel_map[idx][i][1]);

			for (i = 0; i < ARRAY_SIZE(max98937_reg_common_map);
			     i++)
				regmap_write(max98927->regmap[id->driver_data],
					     max98937_reg_common_map[i][0],
					     max98937_reg_common_map[i][1]);
			break;
		default:
			max98927->bIsMax98937[id->driver_data] = UNKNOWN_ID;
			presence = 0; // unknow chip ignore register it.
			dev_err(&i2c->dev, "Failed to find smartpa \n");
			break;
		}
	}
	...
	if (presence) {
		i2c_states |=
			presence; //mark this chip, then app can address it.
		if (max98927->dev == NULL) {
			dev_set_name(
				&i2c->dev, "%s",
				"max98927"); //rename the i2c clinet name for easy to use.
			ret = snd_soc_register_codec(&i2c->dev,
						     &soc_codec_dev_max98927,
						     max98927_dai,
